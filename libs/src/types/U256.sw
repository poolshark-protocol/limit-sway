    ...
    fn multiply(self, other: Self) -> Self {
        ...
        if self.a != 0 {
            ...
            if self.b != 0 {
                ...
                assert(other.b == 0);
                let result_b_d = self.b.overflowing_mul_custom(other.d);
                let result_c_d = self.c.overflowing_mul_custom(other.d);
                let result_d_c = self.d.overflowing_mul_custom(other.c);
                let result_d_d = self.d.overflowing_mul_custom(other.d);

                // let value: U128 = result_d_d.upper.overflowing_add_custom(result_c_d.lower).into();

                let (overflow_of_c_to_b_1, mut c) = result_d_d.upper.overflowing_add_custom(result_c_d.lower).into();
                let (mut overflow_of_c_to_b_2, c) = c.overflowing_add_custom(result_d_c.lower).into();

                let (overflow_of_b_to_a_0, overflow_of_c_to_b_2) = overflow_of_c_to_b_1.overflowing_add_custom(overflow_of_c_to_b_2).into();

                let (overflow_of_b_to_a_1, mut b) = result_b_d.lower.overflowing_add_custom(result_c_d.upper).into();
                let (overflow_of_b_to_a_2, b) = b.overflowing_add_custom(result_d_c.upper).into();
                let (overflow_of_b_to_a_3, b) = b.overflowing_add_custom(overflow_of_c_to_b_2).into();
                ...
                // `other.b * 2 ^ (64 * 2) * self.b * 2 ^ (62 ^ 2) > 2 ^ (64 * 4)`.
                assert(self.b == 0);
                let result_b_d = other.b.overflowing_mul_custom(self.d);
                let result_c_d = other.c.overflowing_mul_custom(self.d);
                let result_d_c = other.d.overflowing_mul_custom(self.c);
                let result_d_d = other.d.overflowing_mul_custom(self.d);
                ...
                let result_c_c = other.c.overflowing_mul_custom(self.c);
                let result_c_d = self.c.overflowing_mul_custom(other.d);
                let result_d_c = self.d.overflowing_mul_custom(other.c);
                let result_d_d = self.d.overflowing_mul_custom(other.d);
                ...
                let (overflow_of_b_to_a_1, mut b) = result_c_c.lower.overflowing_add_custom(result_c_d.upper).into();
                let (overflow_of_b_to_a_2, b) = b.overflowing_add_custom(result_d_c.upper).into();
                let (overflow_of_b_to_a_3, b) = b.overflowing_add_custom(overflow_of_c_to_b_2).into();
